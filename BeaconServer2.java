			...
			while(true) {
				...
				for(Business b : businesses) {
					...
					for(int i = 0; i < photosLeft.size(); i ++) {
						pw = photosLeft.get(i);
						if(distance(b.getLocation().lat(), b.getLocation().lon(), pw.getOrigin().lat(), pw.getOrigin().lon()) < 0.028) {
							thePhotos.add(pw);
							photosLeft.remove(i);
							i --;
						}
				...
				for(int i = 0; i < photosLeft.size(); i ++) {
					...
					for(int j = 0; j < photosLeft.size(); j ++) {
						...
						if(distance(pw1.getOrigin().lat(), pw1.getOrigin().lon(), pw2.getOrigin().lat(), pw2.getOrigin().lon()) < 0.028) {
							onesToTake.add(pw2);
							photosLeft.remove(j);
							j --;
						}
					...
					Beacon newBeacon = new Beacon(onesToTake, (Beacon.last_ID++), pw1.getOrigin(), pw1.getImage());
					newBeaconGroups.add(newBeacon);
				}
